...
async function ensureRootAdminMigration(){
  ...
  if(candidates.length){
    candidates.sort((a,b)=>{
      const sa=Number(a.owned_count||0)+Number(a.member_count||0);
      const sb=Number(b.owned_count||0)+Number(b.member_count||0);
  ...
  if(!canonical){
    ...
    canonical={id:fixedId,root_admin_migration:ROOT_ADMIN_MIGRATION};
    candidates=[canonical];
  }

  const adminId=canonical.id;
  const duplicates=candidates.filter(x=>String(x.id)!==String(adminId));
  for(const d of duplicates){
    // Giữ nguyên bản ghi lịch sử nhưng chuyển quyền sở hữu/thành viên sang tài khoản quản trị chuẩn.
  ...
  // Dọn mọi xung đột username trước khi chuẩn hóa tài khoản gốc.
  ...
  if(Number(canonical.root_admin_migration||0)<ROOT_ADMIN_MIGRATION){
    // Chỉ đặt mật khẩu khởi tạo đúng MỘT LẦN. Sau migration, đổi mật khẩu sẽ không bị reset.
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==="login"){
      ...
      if(username==="admin")await ensureRootAdminMigration();
